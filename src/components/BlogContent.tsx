        ...
        {blogPosts.length === 0 ? (
          <div className="flex flex-col md:flex-row items-center">
            ...
            <div className="md:w-1/2 md:px-4 mt-4 md:mt-0 text-center md:text-left">
              <h3 className="text-3xl md:text-4xl font-bold text-primary">Stay tuned</h3>
              <p className="mt-2 text-gray-700">
                No blog posts here yet, but exciting content is on the way.
              </p>
            </div>
          </div>
        ) : (
          blogPosts.map((post, index) => (
            <div key={index} className="mb-6">
              <div className={`flex flex-col md:flex-row items-center ${index % 2 !== 0 ? 'md:flex-row-reverse' : ''}`}>
                <div className="md:w-1/2 flex justify-center">
                  <Image
                    src={post.imageUrl || "/images/pic.jpeg"}
                    alt={post.title}
                    width={450}
                    height={250}
                    className="object-cover rounded-lg"
